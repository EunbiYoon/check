...
# Session outputs always live under runs/<session>/{lora,eval}/.
# The session name is an explicit RUN_ID when provided, otherwise a UTC
# timestamp generated by the training/evaluation entry point.
...
BLIND_OUTPUT = _env_str(
    "BLIND_OUTPUT", "data/blind-rollout/result/a_beta_all.jsonl"
)
...
# Training-step budget per variant: Hypothesis B runs 200 steps, the A+β mixes
# run 100 (checkpoint every SAVE_STEPS=10, keep the last SAVE_TOTAL_LIMIT=10).
# TRAIN_MAX_STEPS_<VARIANT> overrides one; TRAIN_MAX_STEPS overrides the rest.
_DEFAULT_MAX_TRAIN_STEPS = {
    "filter_on": 200, "filter_off": 200,
    "core": 100, "aux": 100, "all": 100, "rw": 100,
}
VARIANT_MAX_TRAIN_STEPS = {
    variant: _env_optional_int(f"TRAIN_MAX_STEPS_{variant.upper()}", default)
    for variant, default in _DEFAULT_MAX_TRAIN_STEPS.items()
}


def max_train_steps_for_variant(variant: str) -> int | None:
    """Return a variant-specific step cap.

    A shared ``TRAIN_MAX_STEPS`` env wins for every variant; otherwise use the
    per-variant value (``TRAIN_MAX_STEPS_<VARIANT>`` env, else the default —
    200 for Hypothesis B, 100 for the A+β mixes).
    """
    if MAX_TRAIN_STEPS is not None:
        return MAX_TRAIN_STEPS
    return VARIANT_MAX_TRAIN_STEPS.get(variant)
...
PAPER_SAVE_TOTAL_LIMIT = _env_int("SAVE_TOTAL_LIMIT", 10)  # keep the last 10 checkpoints
